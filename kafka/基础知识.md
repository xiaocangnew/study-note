...
- At least one
    消息绝不会丢，但可能会重复传输
    0.11版本之前，producer没有收到commit应答，重发
- Exactly once 
    每条消息肯定会被传输一次且仅传输一次
    0.11开始，新增幂等机制（producer ID + message sequence number）
    Producer配置: enable.idempotence
- Transaction
    0.11开始，支持类似事务的形式发送消息到不同topic/partition
...
* request.required.acks 数据可靠性级别：
  - 1（默认）：在ISR中的leader得到确认。如果leader宕机了，则会丢失数据。
