...
### k8s中的网关
>k8s 网关并非单一组件，而是分为 “原生标准网关”（Gateway API）和 “生态常用网关”（社区成熟工具）两类，二者定位互补，覆盖不同场景需求。
1. Gateway Api 定义了网关的标准化抽象, 提供更灵活、更全面的流量管理能力。
      采用 分层设计，通过多个 CRD（自定义资源）解耦 “网关基础设施” 和 “流量规则配置”，支持多团队协作（如运维管理网关、开发配置路由）。
      1.1. Gateway API 本身是 “标准接口”，需要具体的 “实现工具” 来承载功能（类似 “接口与实现类” 的关系），常用实现包括：
      1.2. Nginx Gateway Fabric：Nginx 官方推出的 Gateway API 实现，轻量、高性能，适合 HTTP/HTTPS/TCP 流量。
      1.3. Istio Gateway：Istio 服务网格基于 Gateway API 实现流量管理，支持灰度发布、链路追踪。
2. k8s 生态常用网关工具
      2.1. Ingress-Nginx（最常用的 HTTP 网关） 早期 Ingress API 的主流实现，专注于 HTTP/HTTPS 流量管理，是 k8s 集群 “入门级网关” 的首选，无流量治理与可观测性。
      2.2. Istio Gateway（服务网格级网关）不仅负责流量转发，还集成了服务网格的可观测性（链路追踪、监控）、流量治理（灰度发布、限流熔断） 等高级能力。
      2.3. Kong Gateway（企业级 API 网关）
