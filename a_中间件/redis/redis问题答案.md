...
1. String类型常用的场景有以下这些：
      ...
      2. 计数功能（INCR 命令原子性，避免并发问题）
      3. 分布式锁：通过 SET key value NX EX 10 实现
2. list列表
...
5. hash的使用场景：
      ...
      2. 购物车
6. Bitmap（位图）：
      1. 海量布尔值。 setbit sign:user:100 0
7. HyperLogLog（去重统计）：
      1. 统计页面uv，pv
8. Geospatial（地理位置）：
...
### sentinel (侧重高可用，看业务场景，在数据量在可预见内不多时用)
...
### redis cluster(侧重水平扩展，在业务场景内，数据量大)
...
1. 更新缓存时使用互斥锁(mutex key):只让一个线程构建缓存，其他线程等待构建缓存的线程执行完(只有得到锁的才能去load)
2. "永远不过期";
