...
### cpu操作是0.1ns纳秒 --3个数量级--> 内存操作是0.1us微秒 --3个数量级-->千兆网卡延时约0.2ms --2个数量级--> 磁盘操作是10ms
本地网络的tcp连接建立，需要0.1ms-1ms之间。

### redis资源层面的三角形
   高计算消耗(lua并发)
   高网络消耗(大value，keys扫表， 大batch操作)  -----> (千兆光纤 1000Mbps == 100MB/s, 获取1MB数据，耗时约10ms， 获取1KB数据耗时约0.1ms)
   高存储消耗(大key)

### redis 常见问题
> “并发与数据一致性问题” “线程阻塞导致的问题” “内存/持久化的配置合理性"
0. 并发与数据一致性问题
      0.1. 缓存穿透：大量请求查询不存在的 Key，Redis直接转发请求到数据库，导致数据库压力暴增，甚至宕机。
                 解决方式： 参数校验 + 缓存空值
      0.2. 缓存击穿
                 解决方式： 热点key过期设置合理 + 缓存更新时加锁方式并发更新
      0.3. 缓存雪崩
                 解决方式： 热点key统一过期设置 + 防止单节点实效方式
      0.4. 与数据库的一致性问题
                 解决方式： cache-aside模式的双删， cache-through模式。
1. 内存/持久化的配置合理性
      1.1. 大key 问题
             1.1.1. 问题点：网络传输慢 && 内存碎片 && cpu过载(遍历慢) && 持久化慢甚至失败。
             1.1.2. 如何解决：  拆分(分层拆分)、压缩、业务优化(缓存分层)。
      1.2. 内存碎片过高
             1.2.1. Redis占用物理内存远大于实际数据内存，导致内存利用率低，间接引发 OOM.  原因：大key问题导致，大量短期 Key 过期后留下碎片
2. 线程阻塞导致的问题
      2.1. 慢查询阻塞
             原因：复杂计算命令耗时长， lua脚本耗时长，执行未加范围限制的集合操作。
      2.2. 后台线程阻塞
              原因： 大量 Key 同时过期（如整点过期缓存），触发主动删除线程繁忙；
                    内存碎片率过高，触发 active-defrag（主动碎片整理）线程占用大量 CPU
      2.3. 网络 IO 阻塞
              原因： 高并发下连接队列满， 网络带宽被占满（如大 Value 传输、主从同步）


### redis 存储分析 [内存膨胀问题](http://www.dataguru.cn/article-267-1.html)
1. redis对于5种类型的存储， 都是使用hash数组形式，称为bucket桶(dictEntry**指针，大小4byte，桶大小是2的n次方)。 
          1.1. key为键，value再分为5种数据类型：string，list，set，zset，hash。
          1.2. 内存大小 = key数量 * (dictEntry 16byte + redisObject 16byte + 包含key的sds大小 + 包含value的sds大小) + bucket个数 * 4byte
2. key存储时，避免重复大量重复前缀 ( hash 结构形式 > 大量重复前缀的string形式)
          2.1. key本身存储占用内存分析 = 元数据+key本身。 其中元数据可能比key还要大。
          2.2. 大量重复前缀的key存储在bucket桶中时，hash映射时可能会发生hash碰撞 && hash值局部集中。
3. 如何减少内存占用,[redis减少内存实战](https://www.qedev.com/bigdata/165762.html)
          3.1. 使用短结构(int, ziplist, embstr)
          3.2. 减少key的个数;
          3.3. 使用bitmap位操作来减少内存占用 



### redis 开发规范
1. QPS不大于20000且容量小于10GB选择主从版本， 其他就集群版。
       1.1、集群版业务要注意事务、Lua脚本、多key操作中保证操作中的key不能跨slot，否则报错；
       1.2、mset、mget获取多个key的操作由于跨slot耗时增加，需要计算key的slot进行分组 
              所以业务使用集群时，mget，hmget获取多个key的操作由于跨slot耗时增加，可以通过先计算key的slot进行分组，然后通过pipeline加速
